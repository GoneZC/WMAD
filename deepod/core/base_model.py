...
class BaseDeepAD(metaclass=ABCMeta):
    ...
    def fit(self, X, y, X_val, y_val, train_loader=None, net=None, criterion=None):
        """
        ...
        for _ in range(self.n_ensemble):
            ...
            self.train_loader, self.net, self.criterion = self.training_prepare(X=self.train_data, y=self.train_label)
            self._training()
    ...
    def _training(self):
        ...
        for i in range(self.epochs):
            ...
            for batch_data in self.train_loader:
                ...
                loss = self.training_forward(batch_data, self.net, self.criterion)
                self.net.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if i == 0 or (i+1) % self.prt_steps == 0:
                scores = self.decision_function(self.val_data)
                auc_pr = average_precision_score(self.val_label, scores)
                
                if self.verbose >= 1:  # 打印输出（受verbose控制）
                    print(f'epoch{i+1:3d}, '
                          f'training loss: {total_loss/cnt:.6f}, '
                          f'auc-pr in validation set: {auc_pr:.6f}, '
                          f'time: {t:.1f}s')
                
                if auc_pr > self.auc_pr:  # 保存最优模型（不受verbose控制）
                    self.auc_pr = auc_pr
                    self.save_model()
                    if self.verbose >= 2:
                        print(f'  → 保存新的最优模型 (AUC-PR: {auc_pr:.6f})')
            if i == 0:
                self.epoch_time = t
