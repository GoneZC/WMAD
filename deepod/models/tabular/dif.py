...
class DeepIsolationForest(BaseDeepAD):
    ...
    def save_model(self, path='./saved_models/dif.pth'):
        # 保存模型状态
        torch.save({
            'net_lst': [net.state_dict() for net in self.net_lst],
            'iForest_lst': self.iForest_lst,
            'x_reduced_lst': self.x_reduced_lst,
            'score_lst': self.score_lst,
        }, path)

    def load_model(self, path='./saved_models/dif.pth'):
        """从指定路径加载模型"""
        checkpoint = torch.load(path)

        # 加载网络列表
        self.net_lst = []
        for state_dict in checkpoint['net_lst']:
            net = MLPnet(
                n_features=self.n_features,
                n_hidden=self.hidden_dims,
                n_output=self.rep_dim,
        ...
        # 加载其他组件
        self.iForest_lst = checkpoint['iForest_lst']
        self.x_reduced_lst = checkpoint['x_reduced_lst']
        self.score_lst = checkpoint['score_lst']
