...
class PReNet(BaseDeepAD):
    ...
    def inference_prepare(self, X):
        ...
        y = self.train_label
        unlabeled_id = np.where((y == 0) | np.isnan(y))[0]
    ...
    def save_model(self, path = './saved_models/prenet.pth'):
        torch.save({
            'net_state_dict': self.net.state_dict(),
        }, path)
        
    def load_model(self,  path = './saved_models/prenet.pth'):
...
class PReNetLoader:
    def __init__(self, X, y, batch_size, steps_per_epoch=None):
        ...
        self.unlabeled_id = np.where((y == 0) | np.isnan(y))[0]
