...
class DeepSAD(BaseDeepAD):
    ...
    def __init__(self, epochs=100, batch_size=64, lr=1e-3,
                 ...
                 eta1=1e-3, eta2=1e-3,verbose=2, random_state=42, model_name='DeepSAD', c = None):
    ...
    def training_prepare(self, X, y):
        ...
        #   use -1 to denote known anomalies, 1 to denote known inliers, and 0 to denote unknown samples
        has_unknown = False
        ...
        criterion = DSADLoss(c=self.c, eta1=self.eta1, eta2=self.eta2)
    ...
    def training_forward(self, batch_data, net, criterion):
    ...
    def _set_c(self, net, dataloader, eps=0.0):
    ...
    def save_model(self, path='./saved_models/dsad.pth'):
        torch.save({
            'net_state_dict': self.net.state_dict(),
            'c': self.c
        }, path)

    def load_model(self,  path='./saved_models/dsad.pth'):
...
class DSADLoss(torch.nn.Module):
    ...
    def __init__(self, c, eta1, eta2 , eps=0, reduction='mean'):
    ...
    def forward(self, rep, semi_targets=None, reduction=None):
        ...
        if semi_targets is not None:
            loss = dist
            loss = torch.where(semi_targets == 1,  self.eta1 * ((dist+self.eps) ** semi_targets.float()), loss)
            loss = torch.where(semi_targets == -1, self.eta2 * ((dist+self.eps) ** semi_targets.float()), loss)
        else:
            loss = dist
