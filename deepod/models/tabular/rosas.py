...
class RoSAS(BaseDeepAD):
    ...
    def save_model(self, path='./saved_models/rosas.pth'):
        torch.save({
            'net_state_dict': self.net.state_dict(),
        }, path)

    def load_model(self,  path='./saved_models/rosas.pth'):
...
class _RoSASLoader:
    def __init__(self, x, y, batch_size=256, steps_per_epoch=None):
        ...
        self.unlabeled_idx = np.where(np.isnan(y))[0]
    ...
    def batch_generation(self):
        this_anchor_idx = np.random.choice(self.norm_idx, self.batch_size, replace=True)
        this_pos_idx = np.random.choice(self.unlabeled_idx, self.batch_size, replace=True)
        this_anom_idx = np.random.choice(self.anom_idx, self.batch_size, replace=True)
