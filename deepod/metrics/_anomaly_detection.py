...
def predict(y_true, y_score):
    """
    Predicts the anomaly label based on the anomaly score. 
    """
    ratio = 100.0 * len(np.where(y_true == 0)[0]) / len(y_true)
    thresh = np.percentile(y_score, ratio)
    y_pred = (y_score >= thresh).astype(int)
...
def evaluate(y_true, y_score):
    """
    Evaluates the model performance by calculating the evaluation metrics.
    """
    y_pred = predict(y_true, y_score)
